...
// Updated export to include sketches
function exportData() {
    const payload = { 
        db: db, 
        sketches: sketches // Bundling drawings into the file
    };
...
// Updated import to handle sketches
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Handle both new bundled format and old single-array format
            if (raw.db && raw.sketches) {
                db = raw.db;
                sketches = raw.sketches;
            } else {
                db = raw; // Legacy support for old files
                sketches = [];
            }
